...
class UpdateEpochCallback(Callback):
    def on_train_epoch_start(self, trainer, pl_module):
        dataset = trainer.datamodule.datasets.get("train", None)
        if hasattr(dataset, "set_epoch"):
            #dataset.set_epoch(trainer.current_epoch, trainer.max_epochs)
            dataset.set_epoch(trainer.current_epoch, 400)
    ...
    try:
        ...
        default_callbacks_cfg = {
            ...
            "update_epoch_callback": {
                "target": "main.UpdateEpochCallback",
            }
        }
        if version.parse(pl.__version__) >= version.parse('1.4.0'):
